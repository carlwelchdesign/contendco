	...
	class API extends REST {
		...
		*/
		...
		private function works(){	
			...
			$query="SELECT distinct c.client_id, c.title, c.thumb, c.publisher_logo, c.project_logo, c.project_title, c.order FROM work c";
			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
		...
		private function work_details(){	
			if($this->get_request_method() != "GET"){
				$this->response('',406);
			}
			$id = (int)$this->_request['id'];
			if($id > 0){	
				$query="SELECT distinct c.details_id, c.client_id, c.title, c.description, c.thumb, c.video FROM work_details c where c.details_id=$id";
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				if($r->num_rows > 0) {
					$result = $r->fetch_assoc();	
					$this->response($this->json($result), 200); // send user details
